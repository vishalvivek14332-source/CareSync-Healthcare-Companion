...
alertRouter.get('/', (req: AuthenticatedRequest, res: Response) => {
  try {
...
alertRouter.put('/:id/review', (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const { id } = req.params;
    const alert = db.prepare('SELECT patient_id FROM alerts WHERE id = ?').get(id) as any;
    if (!alert) return res.status(404).json({ error: 'Alert not found' });

    const patientId = getAuthorizedPatientId(req, res, alert.patient_id);
    if (!patientId) return;
...
alertRouter.post('/sos', (req: AuthenticatedRequest, res: Response) => {
  try {
    const patientId = getAuthorizedPatientId(req, res);
    if (!patientId) return;
    ...
    `).run(id, patientId, patientName, `Emergency button triggered by ${patientName} (${reason}). Caregiver & contacts notified.`, now);
